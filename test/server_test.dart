...
void main() {
  test('GET - In Process - User', () async {
    ...
    expect(
      response.body,
      jsonEncode(const User(login: 'jim', id: '123').toJson()),
    );
  ...
  test('GET - Out of Test Process - App', () async {
    ...
    expect(
      response.body,
      jsonEncode(const App(name: 'Bigapp', id: '543').toJson()),
    );
    p.kill();
  });
  ...
  test('GET - In Process - 404', () async {
    ...
    final uri = 'http://${server.address.host}:${server.port}/foobar';
  ...
  test('POST - In Process - User', () async {
    const user = User(login: 'bob', id: '123');
    final server = await (Router()
          ..addPost(
            '/adduser/<login>',
            (r, arg) async => user,
  ...
  test('PUT - In Process - User', () async {
    ...
    final response = await put(
      ...
      body: const User(login: 'jim', id: '123').toJson(),
    );
  ...
  test('DELETE - In Process', () async {
    ...
    final server = await (Router()
          ..addDelete(
            '/updateuser/<login>',
            (r, arg) async => arg == 'jim' && r.headers['a'] == 'a',
            (success) => success ? responseBody : {},
          ))
        .toServer(8083);
